      ...
      <div className="min-h-screen bg-background flex items-center justify-center px-4">
        <div className="text-center max-w-md">
          ...
          <p className="text-sm text-dark-text/60 mb-2">
            Thank you for shopping with Guru Ji Collection.
          </p>
  ...
  return (
    <div className="min-h-screen bg-background">
            ...
            {(["address", "payment", "review"] as ("address" | "payment" | "review")[]).map((s, i) => (
              <div key={s} className="flex items-center gap-2">
                <div className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold transition-colors ${
                  ...
                  (["address", "payment", "review"].indexOf(step) > i ? "bg-green-500 text-white" : "bg-border text-dark-text/50")
                }`}>
                  {i + 1}
                </div>
                <span className={`text-xs font-medium capitalize ${step === s ? "text-rose-gold" : "text-dark-text/50"}`}>{s}</span>
      ...
      <div className="max-w-5xl mx-auto px-4 py-10 grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div className="bg-white border border-border rounded-xl p-6">
              ...
              <div className="space-y-4">
                <div className="p-3 bg-background-secondary rounded-lg">
                  ...
                  <p className="text-sm text-dark-text/80">{form.name}</p>
                ...
                <div className="p-3 bg-background-secondary rounded-lg">
                  ...
                  <p className="text-sm text-dark-text/80 capitalize">{paymentMethod === "cod" ? "Cash on Delivery" : paymentMethod === "upi" ? "UPI Payment" : "Card / Net Banking"}</p>
                </div>
              </div>
              <div className="flex gap-3 mt-6">
                <button onClick={() => setStep("payment")} className="btn-outline flex-1">Back</button>
                <button onClick={handleOrder} className="btn-primary flex-1">Place Order</button>
              </div>
            </div>
          )}
        </div>
        ...
        {/* Right: Summary */}
        <div className="bg-white border border-border rounded-xl p-5 h-fit sticky top-24">
          ...
          <div className="space-y-2 text-sm">
            ...
            <div className="flex justify-between text-dark-text/70">
              <span>Shipping</span><span>{shipping === 0 ? <span className="text-green-600">Free</span> : formatPrice(shipping)}</span>
            </div>
