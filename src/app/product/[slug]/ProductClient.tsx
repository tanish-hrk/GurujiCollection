      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
          ...
          <div className="space-y-5">
            <div>
              <Link href={`/shop/${product.category}`} className="text-xs text-rose-gold uppercase tracking-widest font-medium hover:text-hover">
                {product.category}
              </Link>
              <h1 className="font-playfair text-3xl text-dark-text mt-1 leading-tight">{product.name}</h1>
              <div className="flex items-center gap-2 mt-2">
                <div className="flex gap-0.5">
                  {Array.from({ length: 5 }).map((_, i) => (
                    <Star key={i} size={14} className={i < Math.round(product.rating) ? "text-amber-400 fill-amber-400" : "text-border"} />
                  ))}
                </div>
                <span className="text-xs text-dark-text/60">{product.rating} ({product.reviews} reviews)</span>
              </div>
            </div>

            <div className="flex items-center gap-3">
              <span className="font-playfair text-3xl font-bold text-rose-gold">{formatPrice(product.salePrice)}</span>
              <span className="text-base text-dark-text/40 line-through">{formatPrice(product.price)}</span>
              {discount >= 5 && (
            ...
            <div className="flex items-center gap-4">
              <div className="flex items-center border border-border rounded">
                <button onClick={() => setQuantity((q) => Math.max(1, q - 1))} className="px-3 py-2 hover:bg-nude-beige transition-colors text-dark-text">−</button>
                <span className="px-4 py-2 text-sm font-medium border-x border-border">{quantity}</span>
                <button onClick={() => setQuantity((q) => q + 1)} className="px-3 py-2 hover:bg-nude-beige transition-colors text-dark-text">+</button>
              </div>
            ...
            <div className="flex flex-col gap-3">
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => { toggleItem(product); toast.success(inWishlist ? "Removed from wishlist" : "Added to wishlist!"); }}
                  className={`flex-1 flex items-center justify-center gap-2 py-3 border rounded-sm text-sm font-medium uppercase tracking-wide transition-all ${
                    inWishlist ? "border-rose-gold bg-rose-gold/5 text-rose-gold" : "border-border text-dark-text/70 hover:border-rose-gold hover:text-rose-gold"
                  }`}
                >
            ...
            {["description", "size-guide", "reviews"].map((tab) => (
              <button
                ...
                className={`px-6 py-3 text-sm font-medium capitalize tracking-wide transition-all border-b-2 -mb-px ${
                  activeTab === tab ? "border-rose-gold text-rose-gold" : "border-transparent text-dark-text/60 hover:text-dark-text"
                }`}
              >
                {tab.replace("-", " ")}
              </button>
          ...
          <div className="py-8">
                  ...
                  <thead>
                    <tr className="bg-background-secondary">
                      {["Size", "Chest (in)", "Waist (in)", "Hip (in)", "Length (in)"].map((h) => (
                        <th key={h} className="px-4 py-2 text-left border-b border-border font-semibold text-dark-text">{h}</th>
                      ))}
                    </tr>
                  </thead>
                  <tbody>
                    {[["XS","32","26","34","44"],["S","34","28","36","44"],["M","36","30","38","44"],["L","38","32","40","45"],["XL","40","34","42","45"],["XXL","42","36","44","46"]].map((row) => (
                      <tr key={row[0]} className="hover:bg-background-secondary/50">
                        {row.map((cell, i) => (
                          <td key={i} className="px-4 py-2 border-b border-border text-dark-text/70">{cell}</td>
                        ))}
                      </tr>
                    ))}
                  </tbody>
            ...
            {activeTab === "reviews" && (
              <div className="space-y-4 max-w-2xl">
                ...
                <p className="text-sm text-dark-text/60 italic">Reviews feature coming soon. Be the first to review this product!</p>
              </div>
            )}
          </div>
        </div>
